...
The backend uses Vitest 3.2.7 for automated tests. Run `npm test` from `backend/` to execute validation-schema and configuration tests. These tests use fake inputs, so they do not start the API server or access the database.

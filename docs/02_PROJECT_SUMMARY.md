...
The backend uses Vitest 3.2.7, selected because it supports the backend's declared Node 18+ runtime. `npm test` runs the test suite once. The initial test file verifies that registration validation accepts safe input and rejects weak passwords and invalid email addresses, and that UUID validation accepts and rejects the expected values. These tests use only fake in-memory input and do not connect to the database.
